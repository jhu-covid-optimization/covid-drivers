...
analysis_df <- read_csv(
  file = './data/05-analysis_df_rucc_2.csv',
  col_types = cols(
...
# 1. Fit GAM --------------------------------------------------------------
dlnm.main.gam <- readRDS('./data/07-main_gam_rucc_2.rds')
dlnm.main.gam <- gam(
  daily_deaths ~
...
#saveRDS(dlnm.main.gam, file = "./data/07-main_gam_rucc_3.rds")

# Before saving final, add same results from main GAM to list for plots
original <- plot(dlnm.main.gam, pages=1, residuals=FALSE, seWithMean = TRUE)
...
png("./results/07-pacf_mainmod-rucc_3.png",width=10, height=7, units="in", res=300)
pacf(residuals(dlnm.main.gam, type="scaled.pearson"))
dev.off()
...
save(main_model_list, file='./data/07-main_model_list-rucc_3.RData')
